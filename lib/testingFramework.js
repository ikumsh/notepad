...
voodoo = function(testList, startPoint = 0) {
  try {
    for (var i = startPoint; i < testList.length; i++) {
      (testList[i])()
    }
  } catch(err) {
    result((err.name.toString() + ': ' + err.message.toString()), failList)
    failedTests++;
    voodoo(testList, (i+1));
  }
}

result = function(result, list) {
  list.push(result)
}
...
function _assert(test, desc, failMsg) {
  ...
  test ? result(('Test Passed: ' + desc), passList) : result(('Test Failed: ' + desc + "<br> &nbsp;&nbsp;&nbsp;" + failMsg), failList);
}
...
function testError(desc, funct, args = null) {
